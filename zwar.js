...
module.exports.onLoad = async () => {
    ...
    if (!fs.existsSync(dirMaterial + "data.json")) (await axios({
        url: "https://raw.githubusercontent.com/ThanhAli-Official/zwar/mainV2/data.json",
        method: 'GET',
        responseType: 'stream'
    })).data.pipe(fs.createWriteStream(dirMaterial + "data.json"));

    if (!fs.existsSync(dirMaterial + "gun.json")) (await axios({
        url: "https://raw.githubusercontent.com/ThanhAli-Official/zwar/mainV2/gun.json",
        method: 'GET',
        responseType: 'stream'
    ...
    try {
        switch (handleReaction.type) {
            case "upgradeSlotConfirm": {
                ...
                return api.sendMessage(`[ 𝗭𝗪𝗔𝗥 ] » Bạn đã mua thành công ${handleReaction.choose} vị trí`, event.threadID, event.messageID);
            }
            default:
                break;
        }
        ...
        console.log(e);
        return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Zombie đã tiêu diệt hết người chơi", event.threadID, event.messageID);
    }
}
    ...
    switch (handleReply.type) {
            ...
            switch (event.body) {
                case "1": {
                    ...
                    return api.sendMessage(
                        "======= 𝗭𝗪𝗔𝗥 =======\n\n" +
                        entryList.join("\n") +
                        "\n\nPhản hồi tin nhắn này với vũ khí bạn muốn mua"
        ...
        case "buyShop": {
            try {
                ...
                var userData = (await Currencies.getData(event.senderID));
                if (isNaN(event.body)) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Lựa chọn của bạn không phải là một con số", event.threadID, event.messageID);
                if (choose > datagun.length || choose < datagun.length) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Lựa chọn của bạn vượt quá danh sách", event.threadID, event.messageID);
                const gunUserChoose = datagun.gun[choose - 1];
                if (userData.money < gunUserChoose.price) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn không đủ tiền để có thể súng mới", event.threadID, event.messageID);
                ...
                return api.sendMessage(`[ 𝗭𝗪𝗔𝗥 ] » Bạn đã mua thành công ${gunUserChoose.name} với giá ${gunUserChoose.price}$`, event.threadID, event.messageID);
            }
            catch (e) {
                console.log(e);
        ...
        case "upgradeSlot": {
            try {
                ...
                if (userData.money < moneyOfUpgrade) return api.sendMessage(`[ 𝗭𝗪𝗔𝗥 ] » Bạn không đủ tiền để có thể mua thêm chỗ cho túi đồ, bạn còn thiếu ${moneyOfUpgrade - userData.money}$`, event.threadID, event.messageID);
                return api.sendMessage(`[ 𝗭𝗪𝗔𝗥 ] » Bạn muốn mua ${choose} với giá ${moneyOfUpgrade} không?\n\nReaction tin nhắn này để đồng ý`, event.threadID, (error, info) => {
...
module.exports.run = async function ({ api, event, args, client, Currencies, Users }) {
    ...
    switch (args[0]) {
        ...
        case "-r": {
            try {
                if (Object.entries(dataUser).length != 0 && dataUser['new']) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn đã có mặt trên chiến trường", event.threadID, event.messageID);
                userData.data.zwar = {};
                ...
                return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn đã đăng ký vào chiến trường thành công", event.threadID, event.messageID);
            }
            catch (e) {
                console.log(e);
                return api.sendMessage("Đã xảy ra lỗi...", event.threadID, event.messageID);
        ...
        case "shop":
        case '-s': {
            if (Object.entries(dataUser).length == 0 && !dataUser['new']) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn chưa có mặt trên chiến trường", event.threadID, event.messageID);
            return api.sendMessage(
                "======= 𝗭𝗪𝗔𝗥 =======" +
                "\n[1] Mua Súng" +
                "\n[2] Bán Zombie" +
                "\n[3] Nâng Cấp Kho" +
                "\n\nPhản hồi tin nhắn này và đưa ra lựa chọn của bạn"
                , event.threadID, (error, info) => {
                    global.client.handleReply.push({
                        name: this.config.name,
                        messageID: info.messageID,
      ...
      case 'help': {
        ...
        default: {
            try {
                ...
                if (dataUser.weapon.price === 0) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn chưa có súng", event.threadID, event.messageID);
                else if (dates < dataUser.weapon.time) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Bạn đang trong thời gian chờ, hãy thử lại sau", event.threadID, event.messageID);
                else if (dataUser.weapon.duribility < 1) {
                    dataUser.weapon = emptygun;
                    return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Súng của bạn đã hỏng, hãy mua súng mới", event.threadID, event.messageID);
                }
                ...
                if (!zombieData || zombieData.length == 0) return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Hiện tại không có zombie để bắn", event.threadID, event.messageID);
                ...
                } else return api.sendMessage("[ 𝗭𝗪𝗔𝗥 ] » Túi của bạn không còn đủ không gian lưu trữ", event.threadID, event.messageID);
                dataUser.weapon.duribility--;
                await Currencies.setData(event.senderID, { data: userData.data });
                const nameUser = (await Users.getData(event.senderID)).name || (await Users.getInfo(event.senderID)).name;

                return api.sendMessage(
                    "[ 𝗭𝗪𝗔𝗥 ] » Bạn đã bắt được " + caught.name +
                    "\n\n====== 𝗧𝗛𝗢̂𝗡𝗚 𝗧𝗜𝗡 ======\n" +
                    `\n👤 Người bắt: ${nameUser}`
