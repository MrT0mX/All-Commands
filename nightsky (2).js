...
if (!text)
    return api.sendMessage("Add text lmao", event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://api.botcahx.biz.id/api/photooxy/night-sky?apikey=f70453e8&text=${text}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}
