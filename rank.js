...
module.exports.makeRankCard = async (data) => {    
    ...
    */
	...
	ctx.textAlign = "start";
	ctx.fillText("/ " + expNextLevel, 780 + ctx.measureText(expCurrent).width + 10, 270);
	ctx.fillStyle = "#FF0099";
	ctx.fillText(expCurrent, 780, 270);
...
module.exports.onLoad = async function () {
    ...
    if (!existsSync(resolve(__dirname, 'cache', 'regular-font.ttf'))) await downloadFile("https://github.com/TuanDeepTry-14072003/font/blob/main/iCielBCDowntown-Regular.otf?raw=true", resolve(__dirname, 'cache', 'regular-font.ttf'));
	if (!existsSync(resolve(__dirname, 'cache', 'bold-font.ttf'))) await downloadFile("https://github.com/tdunguwu/font/blob/main/SVN-Transformer.ttf?raw=true", resolve(__dirname, 'cache', 'bold-font.ttf'));
	if (!existsSync(resolve(__dirname, 'cache', 'rankcard.png'))) await downloadFile("https://i.imgur.com/MjhSYVM.png", resolve(__dirname, 'cache', 'rankcard.png'));
}
